...
const title = document.querySelector('.details-popup-form-head span'),
      ...
      angleQ = document.querySelector('.details-popup-form-body__angle .details-popup-form-body__q')
      anglePrice = document.querySelector('.details-popup-form-body__angle .details-popup-form-body__price')
...
btnAdd.addEventListener('click', () => {
  ...
  let hight = document.querySelector('.size-settings-order__hight input'),
      ...
      angular = document.querySelector('.settings-order__angular input');
  ...
  if (angular.value < 0 || width.value == '' || isNaN(width.value)) {
  ...
  if (check == 0) {
    ...
    if (trowellingRow.classList.contains('_active')) {
      ...
      trowellingQ.textContent = (4).toLocaleString();
      trowellingPrice.textContent = (4 * backConstValue.trowelling[trowellingIndex]).toLocaleString();
    } else {
    ...
    if (glue.classList.contains('_active')) {
      ...
      glueQ.textContent = (2).toLocaleString();
      gluePrice.textContent = (2 * backConstValue.glue).toLocaleString(); 
    } else {
    ...
    // sizeSum = (width.value * hight.value / 10000 - 0.2 * backConstValue.packaging.angle) * 1.1 + Math.cail((width.value * hight.value / 10000 - 0.2 * backConstValue.packaging.angle) * 1.1 / backConstValue.packaging.tile)*backConstValue.packaging.tile;
    sizeQ.textContent = (width.value * hight.value).toLocaleString();
    sizePrice.textContent = (backConstValue.s * width.value * hight.value).toLocaleString();

    angleQ.textContent = (angular.value).toLocaleString();
    anglePrice.textContent = (angular.value * backConstValue.angle.price).toLocaleString();

    numTotal.textContent = (backConstValue.s * width.value * hight.value / 10000 + angular.value * backConstValue.angle.price + 4 * backConstValue.trowelling[trowellingIndex] + 2 * backConstValue.glue.price).toLocaleString();
  }
